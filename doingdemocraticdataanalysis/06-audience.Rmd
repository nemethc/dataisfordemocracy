...
## The audience is a peer

## The audience is your manager
## Show and Tell- Use highlights and captions. Design your data visualization. Please god don't use the defaults
...
## The audience is a policy/decisionmaker
...
## My audience is all of the above- An Introduction to Literate Data Analysis

That brings me to what form democratic data analysis would take in the World-According-To-Corban. Another tool for presenting data analysis to decision-makers are interactive notebooks that merge explanatory text, data, code, and graphics in one file that can be tailored for a variety of audiences. I call this approach ***literate data analysis***^[derived from Literatre Programming, but specific to subject matter expert data analysis]. Code-and-text driven notebooks are becoming exceedingly common in academic domains. In fact, this handbook is an example of an interactive HTML document written in `r`^[using Rmarkdown files and the [bookdown](bookdown.org) package], but they certainly do not have to be this involved. Interactive notebooks combine text, code, and output in one place. They can be structured so that the file shows either plain-text analysis and charts in a web browser, but all of the code that generates it if you open the file in an editor such as RStudio. For example, here is an R notebook that I authored for a research project on state pension funding, and I'll link to other interactive notebooks in the Practices section.  
